...
public class QuizEvalServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		boolean inappropriate = Boolean.parseBoolean("inappropriate");
		boolean userSetDislike = Boolean.parseBoolean("userSetDislike");
		...
		if (!userWantsPractice) {
			...
			q.addScore(qs);
			if(userSetDislike){
				q.setInappropriate(signedInUser, inappropriate);				
			}
			session.setAttribute("quizScore", qs);
		}
		else {
		...
		// REDIRECT TO quizScore.jsp (mainly for multiple page fixes)
		if (userWantsPractice) {	
			session.setAttribute("score", score);
			session.setAttribute("timeElapsed", timeEnd - timeStart);
			session.setAttribute("userWantsPractice", true);
		}
		else {
			out.write("problems?");
			session.setAttribute("quizScore", qs);
			session.setAttribute("timeElapsed", qs.getTimeToComplete());
			session.setAttribute("userWantsPractice", false);
		}
		RequestDispatcher dispatch = request.getRequestDispatcher("quizScore.jsp?quizName=" + q.getName() + "&userName=" + signedInUser.getUserName()); 
