...
public class Quiz extends Object{
	...
	public String getHTML() {
		String result = "";
		result += "<a href=\"quizSummary.jsp?quizName=" + quizName + "\">" + quizName + "</a>";
		return result;
		}

	public static String getQuizDetailsHTML(Quiz quiz){
		String str = "";  
		str += "Quiz Name:" + quiz.getHTML() + " ";
		str += "Creator:" + quiz.getCreator().getHTML() + " ";
		str += "Date Created:" + quiz.getDateCreatedString() + " ";
		return str;
	}
	
	public static String getHTMLBullets(ArrayList<Quiz> quizzes, int num){
		String str = "";
		str += "<ul>";
		for(int i=0; i<quizzes.size(); i++){
			if(num == -1 || i < num){
				str += "<li>";
				str += getQuizDetailsHTML(quizzes.get(i));
				str += "</li>";
			}
		}
		str += "</ul>";
	...
	public static String getHTMLTable(ArrayList<Quiz> quizzes, int num){
		...
		for(int i=0; i<quizzes.size(); i++){
			str += "<tr>";
			if(i >= num) break;
			str += getHTMLTableRow(quizzes.get(i));
			str += "</tr>";
		}
		str += "</table>";
		return str;
	}
	public static String getHTMLTableRow(Quiz q){
		String str = "";
		str += "<td>";
		str += q.getHTML();
		str += "</td>";
		str += "<td>";
		str += q.getCreator().getHTML();
		str += "</td>";
		str += "<td>";
		str += q.getDateCreatedString();
		str += "</td>";
		str += "<td>";
		str += q.getNumTakers();
		str += "</td>";
		
		if(q.getHighScore()!= null){
			str += "<td>";
			str += q.getHighScore().getScore();
			str += "</td>";		
		}else{
			str += "<td>";
			str += 0;
	...
	public ArrayList<User> getTakers(long time){
		...
		//System.out.println("Subtract Time = " + time);
		//System.out.println("Current Time = " + date.getTime());
		//System.out.println("Lowest Acceptable time = " + lowestAcceptableTime);
		try{
			state = (Statement) con.createStatement();
			String cmd = "";
			cmd += "SELECT * ";
			...
			while(rs.next()){
				...
				if(user != null && dateTaken >= lowestAcceptableTime){
					takers.add(user);					
				}
			}
		}catch(SQLException ex){
	...
	public double getAverageScore(long time){
		ArrayList<QuizScore> scores = getScores(time);
		int numScores = scores.size();
		double sumScores = 0;
		for(QuizScore score:scores){
	...
	public double getAverageScore(User taker){
		ArrayList<QuizScore> scores = getScores(taker);
		int numScores = scores.size();
		double sumScores = 0;
		for(QuizScore score:scores){
	...
	public long getAverageTimeToTake(){
		ArrayList<QuizScore> scores = getScores();
		int numScores = scores.size();
		long sumTime = 0;
		for(QuizScore score:scores){
			sumTime += score.getTimeToComplete();
		}
		return sumTime/numScores;
	}
	public long getAverageTimeToTake(long time){
		ArrayList<QuizScore> scores = getScores(time);
		int numScores = scores.size();
		long sumTime = 0;
		for(QuizScore score:scores){
	...
	public long getAverageTimeToTake(User taker){
		ArrayList<QuizScore> scores = getScores(taker);
		int numScores = scores.size();
		long sumTime = 0;
		for(QuizScore score:scores){
	...
	public ArrayList<QuizScore> getScores(){
		...
		return QuizScore.orderByScore(quizScores);
	}
	...
	public boolean hasBeenTaken(){
		if (!getScores().isEmpty()) return true;
		return false;
	}
	public ArrayList<QuizScore> getMostRecentScores(){
		ArrayList<QuizScore> quizScores = new ArrayList<QuizScore>();
		try{
			state = (Statement) con.createStatement();
			String cmd = "";
			...
			ResultSet rs = state.executeQuery(cmd);
			while(rs.next()){
				String takerName = rs.getString(DBConstants.T_QUIZSCORE_C_TAKERS);
				User taker = userManager.getExistingUser(takerName);
				long takerTime = rs.getLong(DBConstants.T_QUIZSCORE_C_TIME);
				long dateTaken = rs.getLong(DBConstants.T_QUIZSCORE_C_DATE);
				int score = rs.getInt(DBConstants.T_QUIZSCORE_C_SCORE);
		...
		return QuizScore.orderByDateTaken(quizScores);
	}
	...
	public long getDateTaken(User taker){
		long mostRecentTaken = DEFAULT_DATE_TAKEN;
		
		if(!getTakers().contains(taker)){
			return mostRecentTaken;
		}
		try{
			state = (Statement) con.createStatement();
			String cmd = "";
			...
			while(rs.next()){
				long dateTaken = rs.getLong(DBConstants.T_QUIZSCORE_C_DATE);
				if(dateTaken > mostRecentTaken){
					mostRecentTaken = dateTaken;				
				}
			}
		}catch(SQLException ex){
			ex.printStackTrace();
	...
	public void removeTaker(User user){
	...
	public static ArrayList<Quiz> orderByDateCreated(ArrayList<Quiz> quizzes){
		if(!quizzes.isEmpty()){
			TreeMap<Long,Quiz> dateCreatedMap = new TreeMap<Long,Quiz>();
			for (Quiz quiz:quizzes){
				dateCreatedMap.put(quiz.getDateCreated(), quiz);
			}
			
			quizzes.clear();
			
			if(!dateCreatedMap.isEmpty()){
				Long mostRecentLeft = dateCreatedMap.lastKey();
				while(true){
					quizzes.add(dateCreatedMap.get(mostRecentLeft));
					mostRecentLeft = dateCreatedMap.lowerKey(mostRecentLeft);
